...
void VideoFileWriter::WriteVideoFrame( ImageRgb24^ frame )
{
	WriteVideoFrame( frame, TimeSpan::MinValue );
}

// Writes new video frame to the opened video file
void VideoFileWriter::WriteVideoFrame( ImageRgb24^ frame, TimeSpan timestamp )
{
    CheckIfDisposed( );
	...
	libffmpeg::uint8_t* ptr = reinterpret_cast<libffmpeg::uint8_t*>( static_cast<void*>( frame->Start ) );

	libffmpeg::uint8_t* srcData[4] = { ptr, NULL, NULL, NULL };
	int srcLinesize[4] = { frame->Width * sizeof(Rgb24), 0, 0, 0 };
